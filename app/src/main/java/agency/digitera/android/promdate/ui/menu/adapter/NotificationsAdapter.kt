package agency.digitera.android.promdate.ui.menu.adapter
...
class NotificationsAdapter(
    ...
    inner class ViewHolder(val view: View) : RecyclerView.ViewHolder(view) {
        fun bind(notification: Notification, clickListener: (Notification) -> Unit) {
            with(itemView) {
                title_text.text =
                    resources.getStringArray(R.array.notification_types_array)[notification.type - 1] //TODO: Get max to change
                val bodyText = resources.getStringArray(R.array.notification_messages_array)[notification.type - 1]
                body_text.text =
                    String.format(bodyText, notification.body[0].sender.firstName, notification.body[0].sender.lastName)
