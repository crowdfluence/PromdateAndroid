...
import agency.digitera.android.promdate.data.db.SingleBoundaryCallback
import agency.digitera.android.promdate.data.entities.User
import agency.digitera.android.promdate.ui.tabs.adapters.SingleAdapter
...
class SinglesTabFragment : Fragment(), TabInterface {
    ...
    private fun initializedPagedListBuilder(config: PagedList.Config): LivePagedListBuilder<Int, User> {
        ...
        livePageListBuilder.setBoundaryCallback(
            SingleBoundaryCallback(
                (activity as MainActivity).singlesDb,
                token
            )
        )
